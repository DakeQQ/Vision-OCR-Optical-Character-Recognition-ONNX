"""Build PaddleOCR-VL image-prefill and image-decode ONNX bundles.

This target-owned helper composes exported component graphs after tracing.  It
keeps state tensors first, shares large initializers through one mmap-friendly
external-data file, and transplants one validated optimized Main graph into the
remaining strategy graphs.
"""
    ...
    for phase in PHASES:
        for strategy in STRATEGIES:
            names[f"image_{phase}_{strategy}"] = (
                ...
                f"{''.join(part.capitalize() for part in strategy.split('_'))}.onnx"
            )
...
def _copy_external_ref(
    ...
    if "location" not in external_data:
        raise RuntimeError(
            f"Shared initializer {initializer.name!r} has no data location."
        )
    return reference
...
def save_shared_initializers_from_tensors(
    ...
    """Stream in-memory shared tensor bytes into a deterministic sidecar."""
    path.parent.mkdir(parents=True, exist_ok=True)
    ...
    model = helper.make_model(graph, producer_name="PaddleOCRVL.Shared_Merged")
    model.ir_version = 10
...
def _external_tensor_path(
    source_folder: Path, tensor: TensorProto
) -> tuple[Path, int, int]:
...
def save_shared_initializers_from_external_tensors(
    ...
    """Write shared tensors by streaming external payloads without materializing them."""
    path.parent.mkdir(parents=True, exist_ok=True)
    data_path = path.with_name(path.name + ".data")
...
def write_shared_initializers_from_external_tensors(
    ...
    """Create a shared sidecar from raw or external source initializers."""
    ...
    for source in sources:
        for initializer in source.graph.initializer:
            if (
                ...
                and initializer.data_type
                not in (TensorProto.UNDEFINED, TensorProto.STRING)
            ):
    ...
    save_shared_initializers_from_external_tensors(shared, source_folder, path)
...
def merge_models_no_check(
    ...
    """Merge prefixed graphs without mutating either input or hiding collisions."""
    ...
    merged.producer_name = "PaddleOCRVL.Shared_Merged"
...
def _build_strategy_graph(
    ...
    merged = merge_models_no_check(
        shell, strategy_model, [("logits", f"{strategy}_logits")]
    )
...
def cleanup_shadowed_constants(model: onnx.ModelProto) -> int:
    """Remove only initializers exactly duplicated by a Constant output."""
    initializers = {
...
def _embed_shell_nodes(model: onnx.ModelProto) -> list[onnx.NodeProto]:
    """Return the Embed shell nodes whose representation must match the donor."""
    nodes = [
        node
        for node in model.graph.node
        if node.name.startswith("embed_")
        or any(
            output.startswith("embed_") or output == "embed_text_hidden_states"
            for output in node.output
        )
    ]
    if not nodes:
        raise RuntimeError("Merged graph has no Embed shell nodes to transplant.")
    return nodes
...
def transplant_quantized_main(
    ...
    """Replace Main and Embed with the canonical optimized donor blocks."""
    ...
    donor_embed_nodes = [
        _copy_node_with_remap(node, {}) for node in _embed_shell_nodes(quantized_primary)
    ]
    target_embed_names = {node.name for node in _embed_shell_nodes(target)}
    ...
    for node in result.graph.node:
        if node.name in target_embed_names:
            if not inserted_embed:
                nodes.extend(donor_embed_nodes)
                inserted_embed = True
            continue
    ...
    if not inserted:
        nodes.extend(donor_nodes)
    if not inserted_embed:
        raise RuntimeError("Target Embed shell nodes were not found during transplantation.")

    donor_initializers = {
        item.name: item for item in quantized_primary.graph.initializer
    }
    target_initializers = {item.name: item for item in target.graph.initializer}
    donor_names = _used_inputs(donor_nodes + donor_embed_nodes) & set(donor_initializers)
...
def validate_onnx_path(path: Path) -> None:
    ...
    if fused_norms:
        for node in fused_norms:
            node.domain = "com.paddleocr_vl.runtime"
        model.opset_import.append(helper.make_opsetid("com.paddleocr_vl.runtime", 1))
...
def build_shared_merged_bundle(
    ...
    """Build all six target image graphs and validate before deleting inputs."""
    ...
    if missing:
        raise FileNotFoundError(
            "Cannot build a complete PaddleOCR-VL image bundle; missing: "
    ...
    if shared_data_path.name != shared_path.name + ".data":
        raise RuntimeError(
            "Shared initializer data filename must be the shared model filename plus '.data'."
        )
    ...
    if set(graphs) != expected:
        raise RuntimeError(
            f"Merged image bundle mismatch: expected {sorted(expected)}, got {sorted(graphs)}."
        )
...
def delete_merged_constituents(
    ...
    """Delete only absorbed component graphs after all merged validators pass."""
    folder = Path(folder)
...
def attach_shared_initializers(session_options, shared_model_path: Path):
    """Attach shared memmaps while retaining all Python owners for session lifetime."""
    ...
    for initializer in shared.graph.initializer:
        ...
        if not location:
            raise RuntimeError(
                f"Shared initializer {initializer.name!r} lacks external data."
            )
