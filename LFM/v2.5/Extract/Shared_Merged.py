"""Compose LFM2.5-VL-450M-Extract image ONNX graphs into shared bundles."""
    ...
    for phase in PHASES:
        for strategy in STRATEGIES:
            names[f"image_{phase}_{strategy}"] = (
                f"LLM_Image{phase.capitalize()}{''.join(word.capitalize() for word in strategy.split('_'))}.onnx"
            )
...
def _save_shared_initializers(shared: dict[str, TensorProto], path: Path, source_folder: Path | None = None) -> None:
    path.parent.mkdir(parents=True, exist_ok=True)
    ...
    try:
        with temporary_data_path.open("wb") as data_file:
            for name, tensor in sorted(shared.items()):
                ...
                reference.data_location = TensorProto.EXTERNAL
                for key, value in (("location", data_path.name), ("offset", str(offset)), ("length", str(length))):
                    entry = reference.external_data.add()
                    entry.key = key
                    entry.value = value
    ...
    model = helper.make_model(graph, producer_name="LFM25VL.Shared_Merged")
    model.ir_version = 10
...
def write_shared_initializers(sources: list[onnx.ModelProto], path: Path, minimum_elements: int = MIN_SHARED_INITIALIZER_ELEMENTS) -> dict[str, dict[str, str]]:
...
def write_shared_initializers_from_external_tensors(sources: list[onnx.ModelProto], source_folder: Path, path: Path, minimum_elements: int = MIN_SHARED_INITIALIZER_ELEMENTS) -> dict[str, dict[str, str]]:
    shared = _shareable_initializers(sources, minimum_elements, Path(source_folder))
    for source in sources:
        for initializer in source.graph.initializer:
            if initializer.data_location == TensorProto.EXTERNAL and initializer.data_type not in (TensorProto.UNDEFINED, TensorProto.STRING):
                add_shared_initializer(shared, initializer, Path(source_folder))
    if not shared:
        raise RuntimeError("No initializers were eligible for streaming sharing.")
...
def merge_models_no_check(first: onnx.ModelProto, second: onnx.ModelProto, io_map: list[tuple[str, str]]) -> onnx.ModelProto:
    ...
    merged.producer_name = "LFM25VL.Shared_Merged"
    ...
    for value in list(first.graph.input) + [value for value in second.graph.input if value.name not in mapped_targets]:
    ...
    for value in [item for item in first.graph.output if item.name not in mapped_sources] + list(second.graph.output):
...
def restore_concat_image_shell_boundary(model: onnx.ModelProto) -> bool | None:
    """Restore LFM2.5-VL's ScatterND image-replacement boundary after ORT.

    The native model replaces the positions selected by ``<image>`` with
    projected visual states using ScatterND; it is not a contiguous Concat
    shell like Qwen-derived models. ``None`` lets the common helper try another
    model-family recovery path.
    """
    graph = model.graph
    ...
    names.update(output for node in graph.node for output in node.output if output)
    if canonical_name in names:
        return False

    has_lfm_image_shell = any(
        node.name.startswith("concat_image_")
        or any(output.startswith("concat_image_") for output in node.output)
        for node in graph.node
    )
    if not has_lfm_image_shell:
        return None
    candidates = [
        node
        for node in graph.node
    ...
    if len(candidates) != 1:
        raise RuntimeError(
            "Cannot restore LFM2.5-VL image replacement boundary: expected one "
            f"concat_image ScatterND node, found {len(candidates)}."
        )
...
def _with_embed(source_folder: Path, model_file_names: dict[str, str] | None, embed: onnx.ModelProto | None = None) -> onnx.ModelProto:
    return embed if embed is not None else prefixed(load_model(source_folder / _file_name(model_file_names, "embed")), "embed_")


def _with_rotary(source_folder: Path, phase: str, model_file_names: dict[str, str] | None) -> onnx.ModelProto:
    return prefixed(load_model(source_folder / _file_name(model_file_names, f"rotary_image_{phase}")), f"{phase}_")


def _prefill_shell(source_folder: Path, main: onnx.ModelProto, model_file_names: dict[str, str] | None, embed: onnx.ModelProto | None) -> tuple[onnx.ModelProto, list[onnx.ModelProto]]:
    embed = _with_embed(source_folder, model_file_names, embed)
    concat = prefixed(load_model(source_folder / _file_name(model_file_names, "concat_image")), "concat_image_")
    hidden = merge_models_no_check(
        embed,
        concat,
        [("embed_input_ids", "concat_image_input_ids"), ("embed_text_hidden_states", "concat_image_text_hidden_states")],
    )
    ...
    return merged, [concat, rotary]


def _decode_shell(source_folder: Path, main: onnx.ModelProto, model_file_names: dict[str, str] | None, embed: onnx.ModelProto | None) -> tuple[onnx.ModelProto, list[onnx.ModelProto]]:
    embed = _with_embed(source_folder, model_file_names, embed)
    ...
    rotary.graph.initializer.append(numpy_helper.from_array(np.zeros((1, 1, 1, 1, 1), dtype=mask_dtype), name="decode_zero_attention_mask"))
...
def _build_strategy_graph(source_folder: Path, main: onnx.ModelProto, phase: str, strategy: str, model_file_names: dict[str, str] | None, embed: onnx.ModelProto | None) -> onnx.ModelProto:
    shell, shell_sources = (_prefill_shell if phase == "prefill" else _decode_shell)(source_folder, main, model_file_names, embed)
    strategy_model = prefixed(load_model(source_folder / _file_name(model_file_names, strategy)), f"{strategy}_")
    merged = merge_models_no_check(shell, strategy_model, [("logits", f"{strategy}_logits")])
    outputs = _state_outputs(main)
    kv_seq = "prefill_kv_seq_len" if phase == "prefill" else "decode_kv_seq_len_next"
    if strategy == "greedy":
        outputs.extend(["greedy_token", kv_seq])
    elif strategy == "penalty_greedy":
        outputs.extend(["penalty_greedy_token", "penalty_greedy_save_id_out", kv_seq])
    elif strategy == "sampling":
        outputs.extend(["sampling_token", "sampling_save_id_out", kv_seq])
    else:
...
def _recipe(phase: str, strategy: str):
    def build(source_folder: Path, main: onnx.ModelProto, model_file_names: dict[str, str] | None = None, embed: onnx.ModelProto | None = None) -> onnx.ModelProto:
...
def make_merged_build_plan(model_file_names: dict[str, str] | None = None):
    ...
    for phase in PHASES:
        for strategy in STRATEGIES:
            ...
            plan.append((_file_name(model_file_names, f"image_{phase}_{strategy}"), _recipe(phase, strategy), dependencies))
    return plan
...
def _validate_graph_model(model: onnx.ModelProto) -> None:
    ...
    undefined = sorted({name for node in model.graph.node for name in node.input if name and name not in known and name not in produced})
    if undefined:
...
def validate_onnx_path(path: Path) -> None:
    ...
    fused_norms = [node for node in model.graph.node if not node.domain and node.op_type == "SimplifiedLayerNormalization"]
    if fused_norms:
        for node in fused_norms:
            node.domain = "com.lfm25vl.runtime"
        model.opset_import.append(helper.make_opsetid("com.lfm25vl.runtime", 1))
...
def build_shared_merged_bundle(source_folder: Path, out_folder: Path | None = None, min_shared_elements: int = MIN_SHARED_INITIALIZER_ELEMENTS, model_file_names: dict[str, str] | None = None, delete_constituents: bool = False) -> dict:
    ...
    missing = sorted(dependency for _, _, dependencies in plan for dependency in dependencies if not (source_folder / dependency).exists())
    if missing:
        raise FileNotFoundError("Cannot build a complete LFM2.5-VL bundle; missing: " + ", ".join(dict.fromkeys(missing)))
    ...
    if shared_data_path.name != shared_path.name + ".data":
        raise RuntimeError("Shared initializer data filename must end in '.onnx.data'.")
    ...
    expected = {_file_name(model_file_names, f"image_{phase}_{strategy}") for phase in PHASES for strategy in STRATEGIES}
    ...
    if delete_constituents and out_folder.resolve() == source_folder.resolve():
        result["removed_constituents"] = delete_merged_constituents(source_folder, protected_names=(shared_path.name, shared_data_path.name))
    return result
