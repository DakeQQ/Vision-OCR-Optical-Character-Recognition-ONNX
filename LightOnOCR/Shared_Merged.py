"""Build LightOnOCR image-prefill and image-decode ONNX bundles.
...
def save_shared_initializers_from_tensors(shared: dict[str, TensorProto], path: Path) -> None:
	...
	model = helper.make_model(graph, producer_name="LightOnOCR.Shared_Merged")
	model.ir_version = 10
...
def merge_models_no_check(
	...
	merged.producer_name = "LightOnOCR.Shared_Merged"
...
def _zero_like_model(
	...
	return helper.make_model(
		graph,
		producer_name="LightOnOCR.Shared_Merged",
...
def _target_main_remap(target: onnx.ModelProto) -> dict[str, str]:
	...
	if "decode_rotary_cos" in names:
		...
		if "embed_text_hidden_states" in names:
...
def validate_onnx_path(path: Path) -> None:
	...
	# This ORT build exposes its fused RMSNorm in the default domain, matching
	# QwenVL's export contract, while upstream ONNX has no schema for it. Check a
	...
	if fused_norms:
		for node in fused_norms:
			node.domain = "com.firered.runtime"
		model.opset_import.append(helper.make_opsetid("com.firered.runtime", 1))
...
def build_shared_merged_bundle(
	...
	if missing:
		raise FileNotFoundError(
			"Cannot build a complete FireRed image bundle; missing: " + ", ".join(dict.fromkeys(missing))
		)
