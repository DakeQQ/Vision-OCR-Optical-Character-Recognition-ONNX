"""Shared optimizer for image-only OCR merged ONNX bundles.

Model-specific ``Optimize_ONNX.py`` facades supply their paths, plans, and
local ``Shared_Merged`` implementation through :class:`OptimizerConfig`.
"""
...
class Plan:
    ...
    # Module quantization and node selection
    method:              str                    = "Q4"     # Q2 | Q4 | Q8 | DYNAMIC | F16 | F32 (optimize-only).
    algo:                str  | None            = None     # AFFINE_REFINE_V2 | DEFAULT | RTN | HQQ | k_quant.
    op_types:            tuple[str, ...] | None = None     # e.g. ("MatMul",) or ("Gather",).
    axes:                tuple[int, ...] | None = None     # Quantization axis per op type.
    block_size:          int  | None            = None
    accuracy_level:      int  | None            = None     # MatMulNBits accuracy level for the DEFAULT algorithm.
    ...
    quantize_embedding:  bool | None            = None     # None=algorithm default; False keeps an untied Gather float.
    embedding_residual_dtype: str | None        = None     # F16 | F32 correction for a quantized untied Gather.
    compact_embedding_residual: bool            = False    # Q8 correction lookup instead of a full F16/F32 residual table.

    # Optimization and precision
    optimize:            bool                   = True
    fp16:                bool                   = False

    # KV surgery and storage
    kv_surgery:          bool | str | None      = None     # None inherits global; auto | True | False.
    external:            bool | None            = None     # None inherits; auto-forced when the model exceeds 2 GB.


@dataclass
class OptimizerConfig:
    """Model-specific settings consumed by the shared merged-bundle pipeline."""

    # Bundle and model-plan inputs
    original_folder_path: str
    quantized_folder_path: str
    download_path: str
    shared_merged: ModuleType
    ...
    # Node selection and ONNX graph optimization
    nodes_to_exclude: list[str] | None = None
    nodes_to_include: list[str] | None = None
    ...
    slim_skip_fusion_patterns: list[str] | None = field(
        default_factory=lambda: ["EliminationReshape"]
    )
    ...
    f16_op_block_list: list[str] | None = field(
    ...
    # KV-cache graph surgery
    kv_attention_surgery: bool | str = "auto"
    kv_blocked_qdq_surgery: bool = False


Shared_Merged: ModuleType | None = None
_PRIMARY_MERGED_KEY = "image_prefill_greedy"
_PRIMARY_MERGED_MODEL = ""
_MERGED_MODEL_NAMES: tuple[str, ...] = ()
MODEL_PLANS: dict[str, Plan] = {}
_PRECISION_MODEL_PLANS: list[Plan] = []
...
AFFINE_V2_SEED_CHUNK_BLOCKS = 65536
AFFINE_V2_SEED_BLOCKS_PER_JOB = 1024
...
class ResolvedPlan:
    ...
    embedding_residual_dtype: str | None
    compact_embedding_residual: bool
...
def resolve_plan(plan: Plan) -> ResolvedPlan:
    return ResolvedPlan(
        ...
        quantize_embedding=plan.quantize_embedding,
        embedding_residual_dtype=(
            plan.embedding_residual_dtype.upper()
            if plan.embedding_residual_dtype is not None
            else None
        ),
        compact_embedding_residual=plan.compact_embedding_residual,
...
def validate_plan(name: str, rp: ResolvedPlan) -> None:
    ...
    if rp.embedding_residual_dtype not in (None, *_EMBEDDING_RESIDUAL_DTYPES):
        raise ValueError(
            f"[{name}] embedding_residual_dtype must be one of "
            f"{sorted(_EMBEDDING_RESIDUAL_DTYPES)} or None."
        )
    if rp.embedding_residual_dtype is not None and (
        rp.method not in _WEIGHT_ONLY_BITS
        or rp.algo != "AFFINE_REFINE_V2"
        or rp.quantize_embedding is False
    ):
        raise ValueError(
            f"[{name}] embedding_residual_dtype requires AFFINE_REFINE_V2 "
            "weight-only embedding quantization."
        )
    if rp.compact_embedding_residual and (
        rp.method != "Q8"
        or rp.algo != "AFFINE_REFINE_V2"
        or rp.embedding_residual_dtype is None
    ):
        raise ValueError(
            f"[{name}] compact_embedding_residual requires AFFINE_REFINE_V2 Q8 "
            "embedding quantization with an F16 or F32 source residual."
        )
...
def make_standalone_quantization_plan(
    method: str,
    *,
    algorithm: str = "AFFINE_REFINE_V2",
    ...
    """Build a validated plan for a standalone ONNX graph quantization pass."""
    normalized_algorithm = (
        "k_quant" if algorithm.lower() == "k_quant" else algorithm.upper()
    )
    plan = Plan(
        method=method,
        algo=normalized_algorithm,
        op_types=op_types,
        axes=axes,
    ...
    validate_plan("standalone", resolved)
    return resolved
...
def _affine_v2_numba_kernel():
    ...
    # Model facades load this module through importlib under family-specific
    # names. A persistent Numba cache serializes that dynamic module context and
    # can fail to deserialize in a later export process; the lru_cache above
    # still reuses the compiled kernel for the lifetime of the current process.
...
def _gather_quantization_residual(
    weight: np.ndarray,
    ...
    """Store a compact row-wise correction for a GatherBlockQuantized embedding."""
    rows = weight.shape[0]
    residual = np.empty(weight.shape, dtype=dtype)
    rows_per_chunk = max(1, _K_QUANT_FINAL_CHUNK_VALUES // logical_width)
    for start in range(0, rows, rows_per_chunk):
        end = min(start + rows_per_chunk, rows)
        dequantized = (
            quantized[start:end].astype(np.float32)
            - zero_points[start:end, :, None].astype(np.float32)
        ) * scales[start:end, :, None].astype(np.float32)
        dequantized = dequantized.reshape(end - start, -1)[:, :logical_width]
        residual[start:end] = np.asarray(weight[start:end], dtype=np.float32) - dequantized
    return residual
...
def _quantize_affine_v2_gather(
    ...
    quantized_output = list(node.output)
    replacement_nodes = []
    residual_inputs = None
    residual_output = None
    if rp.embedding_residual_dtype is not None:
        residual_np_dtype, residual_tensor_type = _EMBEDDING_RESIDUAL_DTYPES[
            rp.embedding_residual_dtype
        ]
        quantized_output = [make_name("quantized_output")]
        residual = _gather_quantization_residual(
            weight_array,
            quantized,
            scales.reshape(rows, block_count),
            zero_points.reshape(rows, block_count),
        ...
        if rp.compact_embedding_residual:
            residual_quantized, residual_scales, residual_zero_points, residual_stats = (
                _affine_refine_v2_q4_rows(
                    residual.reshape(rows, logical_width),
                    rp.block_size,
                    rp.symmetric,
                    bits=8,
                )
            )
            total_stats = residual_stats
            residual_logical_quantized = (
                residual_quantized.reshape(rows, -1)[:, :logical_width]
                .reshape(weight_array.shape)
            )
            residual_scales = residual_scales.reshape(
                *outer_shape, block_count
            ).astype(weight_array.dtype, copy=False)
            residual_zero_points = residual_zero_points.reshape(
                *outer_shape, block_count
            )
            residual_weight_name = make_name("residual_weight")
            residual_scale_name = make_name("residual_scales")
            residual_zero_point_name = make_name("residual_zero_points")
            initializers.extend([
                _make_uintn_initializer(
                    residual_weight_name, residual_logical_quantized, 8
                ),
                _make_quant_initializer(residual_scale_name, residual_scales),
                _make_uintn_initializer(
                    residual_zero_point_name, residual_zero_points, 8
                ),
            ])
            residual_inputs = (
                residual_weight_name,
                residual_scale_name,
                residual_zero_point_name,
            )
            residual_output = make_name("residual_quantized_output")
        else:
            residual_name = make_name("residual")
            initializers.append(_make_quant_initializer(residual_name, residual))
            residual_output = make_name("residual_output")
    ...
    if rp.embedding_residual_dtype is not None:
        if residual_inputs is not None:
            replacement_nodes.append(helper.make_node(
                "GatherBlockQuantized",
                [residual_inputs[0], node.input[1], residual_inputs[1], residual_inputs[2]],
                [residual_output],
                name=make_name("residual_gather_q8"),
                domain="com.microsoft",
                gather_axis=gather_axis,
                quantize_axis=quantize_axis,
                block_size=rp.block_size,
            ...
            stats.add(total_stats)
            residual_input = residual_output
        else:
            replacement_nodes.append(helper.make_node(
                "Gather",
                [residual_name, node.input[1]],
                [residual_output],
                axis=gather_axis,
                name=make_name("residual_gather"),
            ))
            residual_input = residual_output
            if residual_tensor_type != weight.data_type:
                residual_input = make_name("residual_cast")
                replacement_nodes.append(helper.make_node(
                    "Cast",
                    [residual_output],
                    [residual_input],
                    to=weight.data_type,
                    name=make_name("residual_cast"),
                ))
        replacement_nodes.append(helper.make_node(
            "Add",
            [quantized_output[0], residual_input],
            list(node.output),
            name=make_name("residual_add"),
        ))
    return replacement_nodes, stats
...
def _gather_node_key(node: onnx.NodeProto) -> tuple[str, tuple[str, ...], tuple[str, ...]]:
    return node.name, tuple(node.input), tuple(node.output)


def _token_embedding_gather_key(graph) -> tuple[str, tuple[str, ...], tuple[str, ...]] | None:
    """Return the one token-embedding Gather eligible for low-bit conversion.

    GatherBlockQuantized is appropriate for a token table only. Position, RoPE,
    shape, and control-flow tables are also commonly implemented as Gather nodes,
    so selecting every constant Gather would silently quantize runtime metadata.
    """
    initializers = _init_map(graph)
    candidates = []
    for node in graph.node:
        ...
        if weight is None or len(weight.dims) != 2:
            continue
        rows, columns = _tensor_dims(weight)
        if rows <= 0 or columns <= 0:
            continue
        index_name = node.input[1].lower()
        identity = " ".join((node.name, node.input[0], *node.output)).lower()
        shell_output = "embed_text_hidden_states" in node.output
        has_embedding_label = "embed" in identity
        token_index = "input_ids" in index_name or "token_ids" in index_name
        if shell_output:
            priority = 3
        elif token_index and has_embedding_label:
            priority = 2
        elif has_embedding_label and ("token" in index_name or "ids" in index_name):
            priority = 1
        else:
            continue
        candidates.append((priority, rows * columns, _gather_node_key(node)))
    if not candidates:
        return None
...
def quantize_affine_v2_model(
    ...
    quantized_matmuls = 0
    quantized_gathers = 0
    skipped_non_embedding_gathers = 0

    def rewrite_graph(graph) -> None:
        nonlocal quantized_matmuls, quantized_gathers, skipped_non_embedding_gathers
        init_map = _init_map(graph)
        make_name = _make_name_factory(graph, f"affine_refine_v2_q{bits}_")
        replaced_initializers: set[str] = set()
        new_nodes = []
        embedding_gather_key = (
            _token_embedding_gather_key(graph)
            if "Gather" in rp.op_types
            else None
        )
        ...
        for node in graph.node:
            ...
            if selected and node.op_type == "MatMul" and len(node.input) >= 2:
                weight = init_map.get(node.input[1])
                if weight is not None:
                    weight_array = tensor_array_loader(weight) if tensor_array_loader else None
                    try:
                        replacement, stats = _quantize_affine_v2_matmul(
                            graph, node, weight, rp, bits, make_name, weight_array
                        )
                    finally:
                        _close_tensor_array(weight_array)
                    if replacement is not None:
                        quantized_matmuls += 1
                        replaced_initializers.add(weight.name)
            elif selected and node.op_type == "Gather" and len(node.input) >= 2:
                if _gather_node_key(node) != embedding_gather_key:
                    skipped_non_embedding_gathers += 1
                else:
    ...
    if skipped_non_embedding_gathers:
        print(
            f"  AFFINE_REFINE_V2: kept {skipped_non_embedding_gathers} non-embedding "
            "Gather node(s) in floating point."
        )
    return total
...
def configure_optimizer(config: OptimizerConfig) -> None:
    """Bind one model family to this shared optimizer module."""
    global Shared_Merged, ORIGINAL_FOLDER_PATH, QUANTED_FOLDER_PATH, DOWNLOAD_PATH
    global QUANT_METHOD, WEIGHT_ONLY_ALGORITHM, BLOCK_SIZE, ACCURACY_LEVEL
    global QUANT_SYMMETRIC, QUANT_FORMAT, AFFINE_V2_SEED_ITERATIONS
    global AFFINE_V2_SEED_ZP_RADIUS, AFFINE_V2_SEED_CHUNK_BLOCKS
    global AFFINE_V2_SEED_BLOCKS_PER_JOB, AFFINE_V2_SEED_WORKERS
    global AFFINE_V2_NUMBA_THREADS, AFFINE_V2_ITERATIONS, AFFINE_V2_CLIP_RATIOS
    global AFFINE_V2_CHUNK_BLOCKS, AFFINE_V2_WEIGHTED_TOLERANCE
    global AFFINE_V2_ASYM_ZP_SWEEP_LIMIT, DYNAMIC_WEIGHT_TYPE
    global DYNAMIC_PER_CHANNEL, DYNAMIC_REDUCE_RANGE, NODES_TO_EXCLUDE
    global NODES_TO_INCLUDE, FORCE_EXTERNAL_DATA, UPGRADE_OPSET, OPTIMIZER_LEVEL
    global OPTIMIZER_MODEL_TYPE, OPTIMIZER_ONLY_ONNXRUNTIME
    global OPTIMIZER_FUSION_OPTIONS, SHAPE_INFER, SLIM_SKIP_FUSION_PATTERNS
    global SLIM_SKIP_OPTIMIZATIONS, SLIM_SIZE_THRESHOLD, F16_KEEP_IO_TYPES
    global F16_FORCE_INITIALIZERS, F16_MIN_POSITIVE_VAL, F16_MAX_FINITE_VAL
    global F16_NODE_BLOCK_LIST, F16_OP_BLOCK_LIST, KV_ATTENTION_SURGERY
    global KV_BLOCKED_QDQ_SURGERY, _PRIMARY_MERGED_MODEL, _MERGED_MODEL_NAMES
    global MODEL_PLANS, _PRECISION_MODEL_PLANS, MIXED_PRECISION

    required_shared_api = (
        "default_model_file_names",
        "MERGED_BUILD_PLAN",
        "make_merged_build_plan",
    )
    missing_shared_api = [
        name for name in required_shared_api if not hasattr(config.shared_merged, name)
    ]
    if missing_shared_api:
        raise TypeError(
            "shared_merged is missing the merged-bundle API: "
            f"{', '.join(missing_shared_api)}."
        )
    if not config.model_plans:
        raise ValueError("model_plans must include the metadata, vision, and merged donor plans.")

    Shared_Merged = config.shared_merged
    ORIGINAL_FOLDER_PATH = str(config.original_folder_path)
    QUANTED_FOLDER_PATH = str(config.quantized_folder_path)
    DOWNLOAD_PATH = str(config.download_path)
    QUANT_METHOD = config.quant_method.upper()
    algorithm = config.weight_only_algorithm
    WEIGHT_ONLY_ALGORITHM = "k_quant" if algorithm.lower() == "k_quant" else algorithm.upper()
    BLOCK_SIZE = config.block_size
    ACCURACY_LEVEL = config.accuracy_level
    QUANT_SYMMETRIC = config.quant_symmetric
    QUANT_FORMAT = config.quant_format.upper()
    AFFINE_V2_SEED_ITERATIONS = config.affine_v2_seed_iterations
    AFFINE_V2_SEED_ZP_RADIUS = config.affine_v2_seed_zp_radius
    AFFINE_V2_SEED_CHUNK_BLOCKS = config.affine_v2_seed_chunk_blocks
    AFFINE_V2_SEED_BLOCKS_PER_JOB = config.affine_v2_seed_blocks_per_job
    AFFINE_V2_SEED_WORKERS = config.affine_v2_seed_workers
    AFFINE_V2_NUMBA_THREADS = config.affine_v2_numba_threads
    AFFINE_V2_ITERATIONS = config.affine_v2_iterations
    AFFINE_V2_CLIP_RATIOS = config.affine_v2_clip_ratios
    AFFINE_V2_CHUNK_BLOCKS = config.affine_v2_chunk_blocks
    AFFINE_V2_WEIGHTED_TOLERANCE = config.affine_v2_weighted_tolerance
    AFFINE_V2_ASYM_ZP_SWEEP_LIMIT = config.affine_v2_asym_zp_sweep_limit
    DYNAMIC_WEIGHT_TYPE = config.dynamic_weight_type.upper()
    DYNAMIC_PER_CHANNEL = config.dynamic_per_channel
    DYNAMIC_REDUCE_RANGE = config.dynamic_reduce_range
    NODES_TO_EXCLUDE = config.nodes_to_exclude
    NODES_TO_INCLUDE = config.nodes_to_include
    FORCE_EXTERNAL_DATA = config.force_external_data
    UPGRADE_OPSET = config.upgrade_opset
    OPTIMIZER_LEVEL = config.optimizer_level
    OPTIMIZER_MODEL_TYPE = config.optimizer_model_type
    OPTIMIZER_ONLY_ONNXRUNTIME = config.optimizer_only_onnxruntime
    OPTIMIZER_FUSION_OPTIONS = config.optimizer_fusion_options
    SHAPE_INFER = config.shape_infer
    SLIM_SKIP_FUSION_PATTERNS = config.slim_skip_fusion_patterns
    SLIM_SKIP_OPTIMIZATIONS = config.slim_skip_optimizations
    SLIM_SIZE_THRESHOLD = config.slim_size_threshold
    F16_KEEP_IO_TYPES = config.f16_keep_io_types
    F16_FORCE_INITIALIZERS = config.f16_force_initializers
    F16_MIN_POSITIVE_VAL = config.f16_min_positive_val
    F16_MAX_FINITE_VAL = config.f16_max_finite_val
    F16_NODE_BLOCK_LIST = config.f16_node_block_list
    F16_OP_BLOCK_LIST = config.f16_op_block_list
    KV_ATTENTION_SURGERY = config.kv_attention_surgery
    KV_BLOCKED_QDQ_SURGERY = config.kv_blocked_qdq_surgery

    _PRIMARY_MERGED_MODEL = Path(
        Shared_Merged.default_model_file_names()[_PRIMARY_MERGED_KEY]
    ).stem
    _MERGED_MODEL_NAMES = tuple(
        Path(name).stem for name, _, _ in Shared_Merged.MERGED_BUILD_PLAN
    )
    MODEL_PLANS = dict(config.model_plans)
    _PRECISION_MODEL_PLANS = [
        plan for name, plan in MODEL_PLANS.items() if name != "LLM_Metadata"
    ]
    MIXED_PRECISION = (
        any(_uses_fp16(plan) for plan in _PRECISION_MODEL_PLANS)
        and not all(_uses_fp16(plan) for plan in _PRECISION_MODEL_PLANS)
    )


def run_optimizer(config: OptimizerConfig) -> None:
    """Configure and run the shared image OCR optimizer."""
    configure_optimizer(config)
    main()


def make_affine_v2_variant_config(
    config: OptimizerConfig,
    quant_method: str,
    output_folder: str | Path,
) -> OptimizerConfig:
    """Clone an AFFINE_REFINE_V2 bundle recipe for an isolated Q4 or Q8 output."""
    quant_method = quant_method.upper()
    if quant_method not in ("Q4", "Q8"):
        raise ValueError(f"AFFINE_REFINE_V2 variants require Q4 or Q8, got {quant_method!r}.")
    if config.weight_only_algorithm.upper() != "AFFINE_REFINE_V2":
        raise ValueError(
            "make_affine_v2_variant_config requires weight_only_algorithm="
            f"'AFFINE_REFINE_V2', got {config.weight_only_algorithm!r}."
        )
    plans = {
        name: (
            replace(plan, method=quant_method)
            if plan.method.upper() in _WEIGHT_ONLY_BITS
            else plan
        )
    ...
    return replace(
        config,
        quantized_folder_path=str(Path(output_folder)),
        model_plans=plans,
...
def model_exceeds_2gb(model_path: str) -> bool:
    ...
    for location in locations:
        ...
        if relative.is_absolute() or ".." in relative.parts:
            raise RuntimeError(f"Unsafe external-data location {location!r} in {model_path}.")
        data_path = Path(model_path).parent / relative
...
def _cuda_unique_name(prefix: str, names: set[str]) -> str:
    candidate = prefix
    suffix = 0
...
def _cuda_graph_tensor_names(graph, inherited_input_types: dict[str, int]) -> set[str]:
    names = set(inherited_input_types)
    for values in (graph.input, graph.output, graph.value_info, graph.initializer):
        names.update(value.name for value in values if value.name)
    for node in graph.node:
        names.update(name for name in (*node.input, *node.output) if name)
    return names


def _cuda_string_attribute(node, name: str) -> str | None:
    for attribute in node.attribute:
        if attribute.name == name and attribute.type == onnx.AttributeProto.STRING:
            return attribute.s.decode("utf-8")
    return None


def _cuda_int_attribute(node, name: str) -> int | None:
    for attribute in node.attribute:
...
def _main_opset_version(model: onnx.ModelProto) -> int:
    for opset in model.opset_import:
...
def _repair_cuda_graph(
    graph,
    inherited_input_types: dict[str, int],
    opset_version: int,
    ...
    visible_input_types = dict(inherited_input_types)
    visible_input_types.update(local_input_types)
    graph_output_names = {value.name for value in graph.output}
    tensor_names = _cuda_graph_tensor_names(graph, visible_input_types)
    ...
    for node in graph.node:
        resize_is_unsafe = (
            node.op_type == "Resize"
            and bool(node.input)
            and visible_input_types.get(node.input[0]) == TensorProto.UINT8
            and _cuda_string_attribute(node, "mode") in ("linear", "cubic")
        )
        if not resize_is_unsafe:
            rewritten_nodes.append(node)
            continue
        if node.output and node.output[0] in graph_output_names:
            repairs["skipped_uint8_resize_outputs"] += 1
            rewritten_nodes.append(node)
            continue

        cast_output = _cuda_unique_name("cuda_safe_resize_input", tensor_names)
        rewritten_nodes.append(helper.make_node(
            "Cast",
            [node.input[0]],
            [cast_output],
            name=_cuda_unique_name("cuda_safe_resize_cast", node_names),
    ...
    if resize_cast_count:
        del graph.node[:]
        graph.node.extend(rewritten_nodes)
        # The input-type change can invalidate optional optimizer-produced types.
        del graph.value_info[:]
        repairs["uint8_resize_casts"] += resize_cast_count

    compress_axis_name = None
    if opset_version >= 13:
        compress_axis_name = _cuda_unique_name("cuda_safe_compress_axis", tensor_names)

    rewritten_nodes = []
    compress_rewrite_count = 0
    for index, node in enumerate(graph.node, start=1):
        compress_axis = _cuda_int_attribute(node, "axis")
        supported_compress = (
            node.op_type == "Compress"
            and (compress_axis is None or compress_axis == 0)
            and len(node.input) == 2
            and len(node.output) == 1
        )
        if node.op_type == "Compress" and not supported_compress:
            repairs["skipped_compress"] += 1
        if not supported_compress:
            rewritten_nodes.append(node)
            continue

        if compress_rewrite_count == 0 and compress_axis_name is not None:
            graph.initializer.append(helper.make_tensor(
                compress_axis_name, TensorProto.INT64, [1], [0]
            ))
        prefix = f"cuda_safe_compress_{index}"
        nonzero_output = _cuda_unique_name(f"{prefix}_nonzero_output", tensor_names)
        indices_output = _cuda_unique_name(f"{prefix}_indices_output", tensor_names)
        squeeze_inputs = (
            [nonzero_output, compress_axis_name]
            if compress_axis_name is not None
            else [nonzero_output]
        )
        squeeze_attributes = {} if compress_axis_name is not None else {"axes": [0]}
        rewritten_nodes.extend((
            helper.make_node(
                "NonZero",
                [node.input[1]],
                [nonzero_output],
                name=_cuda_unique_name(f"{prefix}_nonzero", node_names),
            ),
            helper.make_node(
                "Squeeze",
                squeeze_inputs,
                [indices_output],
                name=_cuda_unique_name(f"{prefix}_squeeze", node_names),
                **squeeze_attributes,
            ),
            helper.make_node(
                "Gather",
                ...
                list(node.output),
                axis=0,
                name=_cuda_unique_name(f"{prefix}_gather", node_names),
            ),
        ))
        compress_rewrite_count += 1

    if compress_rewrite_count:
        del graph.node[:]
        graph.node.extend(rewritten_nodes)
        repairs["compress_rewrites"] += compress_rewrite_count

    for node in graph.node:
        for attribute in node.attribute:
            if attribute.HasField("g"):
                _repair_cuda_graph(
                    attribute.g, visible_input_types, opset_version, repairs
                )
            for subgraph in attribute.graphs:
                _repair_cuda_graph(
                    subgraph, visible_input_types, opset_version, repairs
                )


def apply_cuda_graph_safety_repairs(model: onnx.ModelProto) -> dict[str, int]:
    """Apply exact ONNX rewrites for known CUDA EP image-graph edge cases.

    CUDA linear/cubic Resize fed directly from a uint8 graph input can return
    corrupted values. CUDA Compress can also fail for an empty axis-0 condition.
    Both rewrites leave runtime ORT_ENABLE_ALL enabled.
    """
    repairs = {
        "uint8_resize_casts": 0,
        "compress_rewrites": 0,
        "skipped_uint8_resize_outputs": 0,
        "skipped_compress": 0,
    }
    _repair_cuda_graph(model.graph, {}, _main_opset_version(model), repairs)
    return repairs


def _cuda_repairs_changed(repairs: dict[str, int]) -> bool:
    return bool(repairs["uint8_resize_casts"] or repairs["compress_rewrites"])


def _format_cuda_repairs(repairs: dict[str, int]) -> str:
    details = []
    if repairs["uint8_resize_casts"]:
        details.append(f"uint8 Resize -> float Cast: {repairs['uint8_resize_casts']}")
    if repairs["compress_rewrites"]:
        details.append(f"Compress -> NonZero/Gather: {repairs['compress_rewrites']}")
    if repairs["skipped_uint8_resize_outputs"]:
        details.append(
            "skipped Resize graph outputs: "
            f"{repairs['skipped_uint8_resize_outputs']}"
        )
    if repairs["skipped_compress"]:
        details.append(f"skipped unsupported Compress: {repairs['skipped_compress']}")
    return "; ".join(details) or "no changes"


def apply_cuda_graph_safety_repairs_to_file(model_path: str) -> dict[str, int]:
    """Repair a saved model without materializing or rewriting external tensors."""
    model = onnx.load(model_path, load_external_data=False)
    try:
        repairs = apply_cuda_graph_safety_repairs(model)
        if _cuda_repairs_changed(repairs):
            # Only the graph protobuf changes, so external initializer sidecars
            # retain their existing paths and byte offsets.
            onnx.save(model, model_path)
...
def _model_contains_operator(model_path: str, op_type: str) -> bool:
    model = onnx.load(model_path, load_external_data=False)
    try:
        pending = [model.graph]
        while pending:
...
def _external_tensor_array(tensor: TensorProto, source_folder: Path) -> np.ndarray:
    """Return one tensor without materializing unrelated external initializers."""
    if tensor.data_location != TensorProto.EXTERNAL:
    ...
    if not location:
        raise RuntimeError(f"External tensor {tensor.name!r} has no data location.")
    location_path = Path(location)
    if location_path.is_absolute() or ".." in location_path.parts:
        raise RuntimeError(f"External tensor {tensor.name!r} has an unsafe data location {location!r}.")
    data_path = source_folder / location_path
    if not data_path.is_file():
        raise FileNotFoundError(f"External tensor data is missing: {data_path}")
    try:
    ...
    length = int(external.get("length", expected_bytes))
    if length < expected_bytes:
        raise RuntimeError(
            f"External tensor {tensor.name!r} advertises {length} bytes, expected at least {expected_bytes}."
        )
    if elements == 0:
...
def _close_tensor_array(array: np.ndarray | None) -> None:
    mmap = getattr(array, "_mmap", None)
    if mmap is not None:
        mmap.close()


def _copy_preserved_external_data(src_path: str, dst_path: str) -> None:
    """Keep untouched source tensors addressable until the final shared blob is written."""
    model = onnx.load(dst_path, load_external_data=False)
    try:
        source_folder = Path(src_path).parent.resolve()
        destination_folder = Path(dst_path).parent.resolve()
        output_sidecar = Path(dst_path).name + ".data"
        locations = {
            {entry.key: entry.value for entry in tensor.external_data}.get("location")
            for tensor in _iter_all_data_tensors(model.graph)
            if tensor.data_location == TensorProto.EXTERNAL
        }
        for location in sorted(location for location in locations if location and location != output_sidecar):
            relative = Path(location)
            if relative.is_absolute() or ".." in relative.parts:
                raise RuntimeError(f"Unsafe preserved external-data location {location!r}.")
            source_data = source_folder / relative
            destination_data = destination_folder / relative
            if not source_data.is_file():
                raise FileNotFoundError(f"Missing preserved external-data source: {source_data}")
            if destination_data.exists():
                raise RuntimeError(
                    f"Preserved external-data destination already exists: {destination_data}"
                )
            # ONNX checker rejects hard-linked tensor data as a potential hard-link
            # attack. This temporary copy is replaced by the compact shared blob
            # once tied embedding storage is unified below.
            shutil.copyfile(source_data, destination_data)
    finally:
        del model
        gc.collect()
...
def optimize_onnx_model(model_path: str, num_heads: int, hidden_size: int,
    ...
    if ort_opt_level > 1 and _model_contains_operator(model_path, "HardSigmoid"):
        # Level 2 can serialize FusedConv(activation="HardSigmoid"), but CUDA EP
        # cannot initialize that contrib kernel. Runtime ORT_ENABLE_ALL remains on.
        print("  CUDA compatibility: using offline optimizer level 1 for HardSigmoid.")
        ort_opt_level = 1
...
def quantize_weight_only(src_path: str, dst_path: str, rp: ResolvedPlan, bits: int, external: bool,
    ...
    if rp.algo == "AFFINE_REFINE_V2":
        ...
        streaming_external = model_exceeds_2gb(src_path) and not do_surgery
        if streaming_external:
            print("  Using memory-bounded external-initializer quantization for the large merged donor.")
            model = onnx.load(src_path, load_external_data=False)
            quantize_affine_v2_model(
                model,
                rp,
                bits,
                tensor_array_loader=lambda tensor: _external_tensor_array(
                    tensor, Path(src_path).parent
                ),
            )
        else:
        ...
        _save_model(model, dst_path, external)
        if streaming_external:
            _copy_preserved_external_data(src_path, dst_path)
        del model
        gc.collect()
        return
...
def _nested_graph_references(graph) -> set[str]:
    """Return values referenced by nested control-flow graphs.

    ONNX If/Loop subgraphs can capture a value from an enclosing graph without
    declaring it as a formal subgraph input. Treat those captures as live when
    removing outer nodes or initializers.
    """
    references: set[str] = set()
    for node in graph.node:
        for attribute in node.attribute:
            if attribute.HasField("g"):
                references.update(value.name for value in attribute.g.output if value.name)
                references.update(
                    input_name
                    for nested_node in attribute.g.node
                    for input_name in nested_node.input
                    if input_name
                )
                references.update(_nested_graph_references(attribute.g))
            for subgraph in attribute.graphs:
                references.update(value.name for value in subgraph.output if value.name)
                references.update(
                    input_name
                    for nested_node in subgraph.node
                    for input_name in nested_node.input
                    if input_name
                )
                references.update(_nested_graph_references(subgraph))
    return references
...
def _dead_code_elimination(graph) -> None:
    ...
    while changed:
        changed = False
        used = set(graph_outputs) | _nested_graph_references(graph)
        for node in graph.node:
...
def _drop_unused_initializers(graph) -> int:
    ...
    used.update(value.name for value in graph.output)
    used.update(_nested_graph_references(graph))
...
def _validate_graph_references(model: onnx.ModelProto, model_name: str) -> None:
    ...
    if missing_outputs:
        raise RuntimeError(
            f"Merged graph {model_name!r} has undefined graph output(s): "
            f"{missing_outputs!r}"
        )
...
def _find_embed_gather(graph):
    ...
    selected_key = _token_embedding_gather_key(graph)
    if selected_key is None:
        raise RuntimeError("semantic token embedding Gather with a 2-D initializer was not found")
    for node in graph.node:
        if _gather_node_key(node) != selected_key:
            continue
        init = inits.get(node.input[0])
        if init is None or len(init.dims) != 2:
            raise RuntimeError("semantic token embedding Gather has no 2-D initializer")
        rows, cols = _tensor_dims(init)
        if rows <= 0 or cols <= 0:
            raise RuntimeError("semantic token embedding Gather has invalid initializer dimensions")
        try:
            axis = int(helper.get_node_attr_value(node, "axis"))
        except ValueError:
            axis = 0
        if axis < 0:
            axis += len(init.dims)
        if axis != 0:
            raise RuntimeError(
                f"semantic token embedding Gather {node.name or init.name!r} uses axis={axis}; "
                "tied embedding sharing requires axis=0"
            )
        return node, init.name, rows, cols
    raise RuntimeError("semantic token embedding Gather was not found in the graph")
...
def _restore_embed_shell_boundary(model: onnx.ModelProto) -> bool:
    ...
    graph = model.graph
    canonical_name = "embed_text_hidden_states"
    if any(canonical_name in node.output for node in graph.node):
        return False
    try:
...
def _restore_concat_image_shell_boundary(model: onnx.ModelProto) -> bool:
    """Restore the Concat-Image-to-Main edge after ORT folds its output Cast."""
    graph = model.graph
    canonical_name = "concat_image_concat_hidden_states"
    defined = {value.name for value in graph.input}
    defined.update(init.name for init in graph.initializer)
    defined.update(output for node in graph.node for output in node.output if output)
    ...
    # Some checkpoint families replace image placeholders with ScatterND rather
    # than concatenating a contiguous vision span. Let their local merge helper
    # restore its verified shell boundary before applying the generic Concat
    # recovery below. ``None`` means the helper does not own this graph shape.
    restore_hook = getattr(Shared_Merged, "restore_concat_image_shell_boundary", None)
    if restore_hook is not None:
        restored = restore_hook(model)
        if restored is not None:
            return bool(restored)

    # Text-only merged bundles do not contain a concat-image shell. Keep the
    # strict recovery check below for multimodal graphs that do contain one.
    has_concat_image_shell = any(
        node.name.startswith("concat_image_")
        or any(output.startswith("concat_image_") for output in node.output)
        for node in graph.node
    )
    if not has_concat_image_shell:
        return False
    ...
    candidates = [
        ...
        and node.name.startswith("concat_image_")
        and "concat_image_vision_hidden_states" in node.input
        and len(node.output) == 1
    ]
    if len(candidates) != 1:
        raise RuntimeError(
            "Cannot restore Concat-Image shell boundary: expected one "
            f"concat_image Concat node, found {len(candidates)}."
        )

    old_name = candidates[0].output[0]
    candidates[0].output[0] = canonical_name
    for node in graph.node:
        for index, name in enumerate(node.input):
            if name == old_name:
...
def _restore_prefill_kv_seq_len_shell_boundary(model: onnx.ModelProto) -> bool:
    """Restore the prefill KV-length output after ORT folds its Add output name."""
    graph = model.graph
    canonical_name = "prefill_kv_seq_len"
    if not any(value.name == canonical_name for value in graph.output):
        return False
    ...
    candidates = [
        ...
        if node.op_type == "Add"
        and len(node.input) == 2
        and set(node.input) == {"prefill_ids_len", "prefill_history_len"}
        and len(node.output) == 1
    ]
    if len(candidates) != 1:
        raise RuntimeError(
            "Cannot restore prefill KV-length boundary: expected one "
            "prefill_ids_len + prefill_history_len Add node, found "
            f"{len(candidates)}."
        )

    old_name = candidates[0].output[0]
    if not old_name:
        raise RuntimeError("Cannot restore prefill KV-length boundary: Add has no output.")
    candidates[0].output[0] = canonical_name
    for node in graph.node:
        for index, name in enumerate(node.input):
            if name == old_name:
...
def _configure_embedding_quantization(rp: ResolvedPlan, share_embed_lmhead: bool) -> None:
    ...
    if not share_embed_lmhead and rp.quantize_embedding is not False:
...
def _value_rank(graph, name: str) -> int | None:
...
def _share_float_embed_lmhead(graph, gather, embed_init: str, lmhead, vocab: int, hidden: int) -> dict:
    ...
    if dims == (vocab, hidden):
        ...
        ids_rank = _value_rank(graph, ids)
        if ids_rank is None:
            raise RuntimeError(
                f"cannot share transposed lm_head {shared_weight!r}: token ID rank for {ids!r} is unknown"
            )
...
def _share_nbits_embed_lmhead(graph, gather, embed_init: str, lmhead, vocab: int, hidden: int,
    ...
    inits = _init_map(graph)
    embed_weight = inits.get(embed_init)
    if embed_weight is None:
        raise RuntimeError("tied embedding initializer was not found")
    embed_type = embed_weight.data_type
    ...
    shape_start = _make_axes_initializer(graph, make("shape_start"), [0])
    shape_end = _make_axes_initializer(graph, make("shape_end"), [-1])
    shape_axis = _make_axes_initializer(graph, make("shape_axis"), [0])
    reshape_tail = _make_axes_initializer(graph, make("reshape_tail"), [-1])
    hidden_shape = _make_axes_initializer(graph, make("hidden_shape"), [hidden])
    ...
    def unpack(packed: str, prefix: str) -> str:
        if bits == 8:
            return packed
        ...
        shape = make(f"{prefix}_shape")
        prefix_shape = make(f"{prefix}_prefix_shape")
        reshape = make(f"{prefix}_reshape_shape")
        replacement.extend([
            helper.make_node("Concat", unpacked, [joined], axis=-1, name=make(f"{prefix}_concat")),
            helper.make_node("Shape", [packed], [shape], name=make(f"{prefix}_shape_node")),
            helper.make_node(
                "Slice",
                [shape, shape_start, shape_end, shape_axis],
                [prefix_shape],
                name=make(f"{prefix}_shape_prefix"),
            ),
            helper.make_node(
                "Concat",
                [prefix_shape, reshape_tail],
                [reshape],
                axis=0,
                name=make(f"{prefix}_shape_concat"),
            ),
    ...
    if has_zero_point:
        ...
        if bits == 8:
            ...
            z_axis = _make_axes_initializer(graph, make("z_axis"), [-1])
            zint = make("zint")
            replacement.append(
    ...
    replacement.extend([
        helper.make_node("Cast", [gs], [sf], to=TensorProto.FLOAT, name=make("s_cast")),
        helper.make_node("Unsqueeze", [sf, axm1], [gs1], name=make("s_unsq")),
        helper.make_node("Sub", [qf, zero_operand], [sub], name=make("sub_node")),
        helper.make_node("Mul", [sub, gs1], [deq], name=make("mul_node")),
    ])
    flat = make("output_float") if embed_type != TensorProto.FLOAT else out
    ids_shape = make("ids_shape")
    output_shape = make("output_shape")
    replacement.extend([
        helper.make_node("Shape", [ids], [ids_shape], name=make("ids_shape_node")),
        helper.make_node(
            "Concat",
            [ids_shape, hidden_shape],
            [output_shape],
            axis=0,
            name=make("output_shape_concat"),
        ),
    ])
    replacement.append(
        helper.make_node("Reshape", [deq, output_shape], [flat], name=make("output_reshape"))
    )
    if embed_type != TensorProto.FLOAT:
        replacement.append(
            helper.make_node("Cast", [flat], [out], to=embed_type, name=make("output_cast"))
        )
...
def _use_memory_bounded_affine_v2(
    src_path: str,
    rp: ResolvedPlan,
    do_surgery: bool = False,
) -> bool:
    return (
        rp.method in _WEIGHT_ONLY_BITS
        and rp.algo == "AFFINE_REFINE_V2"
        and not do_surgery
        and model_exceeds_2gb(src_path)
    )
...
def process_model(name: str, rp: ResolvedPlan) -> None:
    ...
    memory_bounded_affine_v2 = _use_memory_bounded_affine_v2(src_path, rp, do_surgery)
    ...
    if (rp.optimize or use_fp16) and memory_bounded_affine_v2:
        print(
            "  Skipping eager ONNX graph optimization for the memory-bounded large donor; "
            "its quantized Main is consolidated directly into the shared initializer bundle."
        )
    elif rp.optimize or use_fp16:
    ...
    cuda_repairs = apply_cuda_graph_safety_repairs_to_file(dst_path)
    if _cuda_repairs_changed(cuda_repairs):
        print(f"  CUDA graph safety: {_format_cuda_repairs(cuda_repairs)}.")
    elif cuda_repairs["skipped_uint8_resize_outputs"] or cuda_repairs["skipped_compress"]:
        print(f"  CUDA graph safety: {_format_cuda_repairs(cuda_repairs)}.")
    ...
    if source_metadata:
        ...
        print(f"  Metadata: restamped {len(source_metadata)} keys onto the optimized model.")

    _validate_affine_v2_output_graph(Path(dst_path), rp)
...
def _save_merged_model(
    ...
    cuda_repairs = apply_cuda_graph_safety_repairs(model)
    if _cuda_repairs_changed(cuda_repairs):
        print(f"  {file_name}: CUDA graph safety: {_format_cuda_repairs(cuda_repairs)}.")
    elif cuda_repairs["skipped_uint8_resize_outputs"] or cuda_repairs["skipped_compress"]:
        print(f"  {file_name}: CUDA graph safety: {_format_cuda_repairs(cuda_repairs)}.")
    ...
    Shared_Merged.validate_onnx_path(out_path)
...
def _load_transplant_donor(primary_path: Path) -> onnx.ModelProto:
    ...
    if _restore_prefill_kv_seq_len_shell_boundary(donor):
        print("  Restored optimized donor prefill KV-length boundary.")
    if _restore_embed_shell_boundary(donor):
        print("  Restored optimized donor Embed/Main boundary before transplantation.")
    if _restore_concat_image_shell_boundary(donor):
        print("  Restored optimized donor Concat-Image/Main boundary before transplantation.")
...
_FLOAT_MATRIX_TYPES = frozenset(
    ...
    for name in ("FLOAT", "FLOAT16", "DOUBLE", "BFLOAT16")
    if hasattr(TensorProto, name)
)


def _iter_graph_nodes(graph):
    """Yield nodes from a graph and its nested control-flow subgraphs."""
    for node in graph.node:
        yield node
        for attribute in node.attribute:
            if attribute.HasField("g"):
...
def _remaining_float_matrix_weights(graph, rp: ResolvedPlan) -> list[str]:
    """Find selected constant float matrix operands that escaped quantization."""
    if "MatMul" not in rp.op_types:
        return []

    remaining: list[str] = []

    def visit(current_graph, inherited_initializers: dict[str, TensorProto]) -> None:
        local_initializers = _init_map(current_graph)
        visible_initializers = dict(inherited_initializers)
        for graph_input in current_graph.input:
            if graph_input.name not in local_initializers:
                visible_initializers.pop(graph_input.name, None)
        visible_initializers.update(local_initializers)

        for node in current_graph.node:
            selected = node.op_type in ("MatMul", "Gemm")
            if rp.nodes_to_include is not None:
                selected = selected and node.name in rp.nodes_to_include
            if rp.nodes_to_exclude is not None and node.name in rp.nodes_to_exclude:
                selected = False
            ...
            for input_index, input_name in enumerate(node.input[:2]):
                weight = visible_initializers.get(input_name)
                if (
                    weight is not None
        ...
        for node in current_graph.node:
            for attribute in node.attribute:
                if attribute.HasField("g"):
                    visit(attribute.g, visible_initializers)
                for subgraph in attribute.graphs:
                    visit(subgraph, visible_initializers)

    visit(graph, {})
    return remaining


def _remaining_float_token_embedding_gathers(graph) -> list[str]:
    """Find semantic token Gather nodes still backed by a float embedding table."""
    remaining: list[str] = []

    def visit(current_graph) -> None:
        candidate_key = _token_embedding_gather_key(current_graph)
        if candidate_key is not None:
            initializers = _init_map(current_graph)
            for node in current_graph.node:
                if _gather_node_key(node) != candidate_key:
                    continue
                table = initializers.get(node.input[0])
                if table is not None and table.data_type in _FLOAT_MATRIX_TYPES:
                    remaining.append(
                        f"{node.name or 'Gather'} <- {table.name}{tuple(table.dims)}"
                    )
                break
...
def _graph_producers(graph) -> dict[str, onnx.NodeProto]:
    return {
        output: node
        for node in graph.node
...
def _is_residual_corrected_embedding(graph, compact_residual: bool = False) -> bool:
    producers = _graph_producers(graph)
    final = producers.get("embed_text_hidden_states")
    if final is None or final.op_type != "Add":
        return False
    stack = [final]
    seen: set[int] = set()
    quantized_gather_count = 0
    found_residual_gather = False
    while stack:
        node = stack.pop()
        if id(node) in seen:
            continue
        seen.add(id(node))
        quantized_gather_count += node.op_type == "GatherBlockQuantized"
        found_residual_gather |= (
            node.op_type == "Gather" and "residual" in node.name.lower()
        )
        stack.extend(
            producer
    ...
    if compact_residual:
        return quantized_gather_count >= 2
    return quantized_gather_count >= 1 and found_residual_gather


def _validate_affine_v2_output_graph(path: Path, rp: ResolvedPlan) -> None:
    """Validate post-optimization AFFINE_REFINE_V2 nodes from the saved graph."""
    if rp.algo != "AFFINE_REFINE_V2" or rp.method not in ("Q4", "Q8"):
        return
    expected_bits = _WEIGHT_ONLY_BITS[rp.method]
    model = onnx.load(str(path), load_external_data=False)
    try:
        nodes = tuple(_iter_graph_nodes(model.graph))
        matmuls = [node for node in nodes if node.op_type == "MatMulNBits"]
        if "MatMul" in rp.op_types:
            if not matmuls:
                raise RuntimeError(
                    f"{path.name} requested AFFINE_REFINE_V2 MatMul quantization "
                    "but contains no MatMulNBits node."
                )
            widths = {_node_int_attribute(node, "bits") for node in matmuls}
            if widths != {expected_bits}:
                raise RuntimeError(
                    f"{path.name} contains MatMulNBits widths {sorted(widths)}; "
                    f"expected only Q{expected_bits}."
                )
            remaining = _remaining_float_matrix_weights(model.graph, rp)
            if remaining:
                preview = "; ".join(remaining[:8])
                suffix = "" if len(remaining) <= 8 else f"; +{len(remaining) - 8} more"
                raise RuntimeError(
                    f"{path.name} retained {len(remaining)} constant float matrix "
                    f"operand(s) after AFFINE_REFINE_V2 Q{expected_bits}: {preview}{suffix}"
                )

        gathers = [
            node for node in nodes if node.op_type == "GatherBlockQuantized"
        ]
        if "Gather" in rp.op_types:
            if not gathers:
                raise RuntimeError(
                    f"{path.name} requested token embedding quantization but contains "
                    "no GatherBlockQuantized node."
                )
            widths = {_node_int_attribute(node, "bits") for node in gathers}
            if widths != {expected_bits}:
                raise RuntimeError(
                    f"{path.name} contains GatherBlockQuantized widths {sorted(widths)}; "
                    f"expected only Q{expected_bits}."
                )
            if rp.embedding_residual_dtype is not None and not _is_residual_corrected_embedding(
                model.graph, rp.compact_embedding_residual
            ):
                raise RuntimeError(
                    f"{path.name} is missing a residual-corrected quantized embedding path."
                )
            remaining = _remaining_float_token_embedding_gathers(model.graph)
            if remaining:
                raise RuntimeError(
                    f"{path.name} retained a floating token embedding after "
                    f"AFFINE_REFINE_V2 Q{expected_bits}: {'; '.join(remaining)}"
                )
    finally:
        del model


def _validate_affine_v2_merged_bundle(
    out_folder: Path,
    available_files: list[str],
    primary_plan: ResolvedPlan,
    shared_model_name: str,
    shared_data_name: str,
    shared_initializer_names: set[str],
    dropped_tied_embedding: str | None,
) -> None:
    """Validate every merged strategy after transplant and shared-data redirection."""
    if primary_plan.algo != "AFFINE_REFINE_V2" or primary_plan.method not in ("Q4", "Q8"):
        return
    shared_data_path = out_folder / shared_data_name
    if not (out_folder / shared_model_name).is_file() or not shared_data_path.is_file():
        raise RuntimeError("AFFINE_REFINE_V2 merged bundle is missing its shared initializer artifacts.")

    for file_name in available_files:
        path = out_folder / file_name
        _validate_affine_v2_output_graph(path, primary_plan)
        model = onnx.load(str(path), load_external_data=False)
        try:
            initializer_names = {initializer.name for initializer in model.graph.initializer}
            if dropped_tied_embedding is not None and dropped_tied_embedding in initializer_names:
                raise RuntimeError(
                    f"{path.name} retains duplicate tied embedding initializer "
                    f"{dropped_tied_embedding!r}."
                )
            for initializer in model.graph.initializer:
                ...
                if external.get("location") == shared_data_name:
                    if initializer.name not in shared_initializer_names:
                        raise RuntimeError(
                            f"{path.name} references {initializer.name!r} in the shared "
                            "data file but it is absent from the shared carrier."
                        )
        finally:
            del model
    print(
        f"  Verified AFFINE_REFINE_V2 Q{_WEIGHT_ONLY_BITS[primary_plan.method]} "
        f"structure across {len(available_files)} merged strategy graph(s)."
    )
...
def _load_unified_primary(
    ...
    model = onnx.load(str(primary_path), load_external_data=False)
    if _restore_prefill_kv_seq_len_shell_boundary(model):
        print("  Restored optimized primary prefill KV-length boundary.")
    info = (
...
def _redirect_streaming_transplant_donor(
    donor: onnx.ModelProto,
    unify_info: dict | None,
    external_by_name: dict[str, dict[str, str]],
) -> None:
    """Retarget a header-only donor after streaming its tensors into shared data."""
    if unify_info is not None and unify_info["dropped"] != unify_info["shared_weight"]:
        _drop_initializers(donor.graph, {unify_info["dropped"]})
    _drop_unused_initializers(donor.graph)
    Shared_Merged.redirect_shared_initializers_to_external(donor, external_by_name)
...
def _source_shell_carrier_extras(
    source_folder: Path,
    out_folder: Path,
    available_files: list[str],
    primary_file: str,
    primary_model: onnx.ModelProto,
    excluded_initializer_names: set[str],
) -> tuple[list[onnx.ModelProto], Path | None]:
    """Stage source-only shell tensors for the rebuilt optimized carrier.

    The optimized primary owns quantized Main weights. Other strategy graphs can
    still own decode- or strategy-specific shell initializers and Constants, so
    stream only those tensors from the source bundle into the new carrier.
    """
    primary_initializer_names = {
        initializer.name for initializer in primary_model.graph.initializer
    }
    extras: list[onnx.ModelProto] = []
    alias_dir: Path | None = None
    aliases: dict[Path, str] = {}

    def remap_external_tensor(tensor: TensorProto) -> None:
        nonlocal alias_dir
        if tensor.data_location != TensorProto.EXTERNAL:
            return
        ...
        if not source_data.is_file():
            raise FileNotFoundError(
                f"Source shell initializer data is missing: {source_data}"
            )
        source_key = source_data.resolve()
        staged_location = aliases.get(source_key)
        if staged_location is None:
            if alias_dir is None:
                alias_dir = Path(tempfile.mkdtemp(prefix=".merged-carrier-source-", dir=out_folder))
            alias_name = f"{len(aliases):04d}-{source_data.name}"
            alias_path = alias_dir / alias_name
            try:
                os.link(source_data, alias_path)
            except OSError:
                shutil.copy2(source_data, alias_path)
            staged_location = f"{alias_dir.name}/{alias_name}"
            aliases[source_key] = staged_location
        for entry in tensor.external_data:
            if entry.key == "location":
                entry.value = staged_location
                break

    for file_name in available_files:
        if file_name == primary_file:
            continue
        source_model = onnx.load(
            str(source_folder / file_name), load_external_data=False
        )
        shell_nodes = [
            node for node in source_model.graph.node if Shared_Merged._node_is_shell(node)
        ]
        shell_inputs = {
            name for node in shell_nodes for name in node.input if name
        }
        extra = onnx.ModelProto()
        extra.graph.name = f"{Path(file_name).stem}_source_shell_extras"
        for initializer in source_model.graph.initializer:
            if (
                initializer.name not in shell_inputs
                or initializer.name in primary_initializer_names
                or initializer.name in excluded_initializer_names
            ):
                continue
            copied = TensorProto()
            copied.CopyFrom(initializer)
            remap_external_tensor(copied)
            extra.graph.initializer.append(copied)
        for node in shell_nodes:
            if node.op_type != "Constant":
                continue
            copied = onnx.NodeProto()
            copied.CopyFrom(node)
            for attribute in copied.attribute:
                if attribute.type == onnx.AttributeProto.TENSOR:
                    remap_external_tensor(attribute.t)
                elif attribute.type == onnx.AttributeProto.TENSORS:
                    for tensor in attribute.tensors:
                        remap_external_tensor(tensor)
            extra.graph.node.append(copied)
        if extra.graph.initializer or extra.graph.node:
            extras.append(extra)
    return extras, alias_dir
...
def build_quantized_merged_bundle(resolved: dict[str, ResolvedPlan]) -> None:
    ...
    primary_source_path = source_folder / primary_file
    primary_surgery = False
    if primary_plan.kv_surgery is not False:
        primary_surgery, _ = plan_kv_surgery(str(primary_source_path))
    memory_bounded_primary = _use_memory_bounded_affine_v2(
        str(primary_source_path), primary_plan, primary_surgery
    )
    ...
    primary_model, unify_info = _load_unified_primary(
        ...
        share_embed_lmhead,
        materialize_external=not memory_bounded_primary,
    )
    dropped_tied_embedding = (
        {unify_info["dropped"]}
        if unify_info is not None and unify_info["dropped"] != unify_info["shared_weight"]
        else set()
    )
    source_shell_extras, source_alias_dir = _source_shell_carrier_extras(
        source_folder,
        out_folder,
        available_files,
        primary_file,
    ...
    try:
        if memory_bounded_primary:
            print("  Streaming shared initializers from the quantized external donor.")
        if source_shell_extras:
            print(
                "  Preserving source-only shell tensors from "
                f"{len(source_shell_extras)} merged strategy graph(s)."
            )
        external_by_name = Shared_Merged.write_shared_initializers_from_external_tensors(
            [primary_model, *source_shell_extras],
            out_folder,
            out_folder / shared_model_name,
        )
    finally:
        del source_shell_extras
        if source_alias_dir is not None:
            shutil.rmtree(source_alias_dir, ignore_errors=True)
    if memory_bounded_primary:
        _redirect_streaming_transplant_donor(donor, unify_info, external_by_name)
    else:
    ...
    if primary_plan.algo == "AFFINE_REFINE_V2" and primary_plan.method in ("Q4", "Q8"):
        shared_carrier = onnx.load(
            str(out_folder / shared_model_name), load_external_data=False
        )
        try:
            _validate_affine_v2_merged_bundle(
                out_folder,
                available_files,
                primary_plan,
                shared_model_name,
                shared_data_name,
                {initializer.name for initializer in shared_carrier.graph.initializer},
                unify_info["dropped"] if unify_info is not None else None,
            )
        finally:
            del shared_carrier
...
def main() -> None:
    if Shared_Merged is None:
        raise RuntimeError(
            "This shared optimizer needs an OptimizerConfig. Run a model-specific "
            "Optimize_ONNX.py facade instead."
        )
...
if __name__ == "__main__":
    raise SystemExit("Run a model-specific Optimize_ONNX.py facade instead.")
