"""Build OvisOCR2's image-prefill and image-decode merged ONNX graphs.

OvisOCR2 has six full-attention KV caches and eighteen linear-attention state
...
from OCR_Shared_Initializers import add_shared_initializer
...
def default_model_file_names() -> dict[str, str]:
    """Return OvisOCR2's complete image-only runtime filename contract."""
    names = {
...
def _file_name(model_file_names: dict[str, str] | None, key: str) -> str:
    ...
    try:
        ...
        raise KeyError(f"Missing OvisOCR2 filename role {key!r}.") from error
...
def _save_shared_initializers(
    ...
    try:
        ...
        temporary_data_path.unlink(missing_ok=True)
        raise
    graph = helper.make_graph([], "ovis_shared_initializers", [], [], initializer=references)
    model = helper.make_model(graph, producer_name="OvisOCR2.Shared_Merged")
    model.ir_version = 10
...
def _shareable_initializers(
    ...
    for source in sources:
        ...
        for node in source.graph.node:
            initializer = _constant_as_initializer(node)
...
def write_shared_initializers(
    ...
    if not shared:
        raise RuntimeError("No OvisOCR2 language initializers were eligible for sharing.")
...
def write_shared_initializers_from_external_tensors(
    ...
    if not shared:
        raise RuntimeError("No OvisOCR2 initializers were eligible for streaming sharing.")
...
def redirect_shared_initializers_to_external(
    ...
    """Redirect matching initializer and Constant payloads to shared storage."""
    ...
    existing_names = {initializer.name for initializer in rewritten}
    retained_nodes: list[onnx.NodeProto] = []
    constant_references: list[TensorProto] = []
    for node in model.graph.node:
        ...
        constant_references.append(_copy_external_ref(initializer, external))
        existing_names.add(initializer.name)
        redirects += 1
    if constant_references:
        del model.graph.node[:]
        model.graph.node.extend(retained_nodes)
        model.graph.initializer.extend(constant_references)
    return redirects
...
def merge_models_no_check(
    ...
    merged.producer_name = "OvisOCR2.Shared_Merged"
...
def transplant_quantized_main(
    ...
    if not all_donor_nodes:
        raise RuntimeError("The canonical OvisOCR2 donor has no Main nodes.")
...
def validate_onnx_path(path: Path) -> None:
    ...
    if fused_norms:
        for node in fused_norms:
            node.domain = "com.ovisocr2.runtime"
        model.opset_import.append(helper.make_opsetid("com.ovisocr2.runtime", 1))
...
def build_shared_merged_bundle(
    ...
    if missing:
        raise FileNotFoundError(
            "Cannot build a complete OvisOCR2 merged bundle; missing: " + ", ".join(missing)
        )
