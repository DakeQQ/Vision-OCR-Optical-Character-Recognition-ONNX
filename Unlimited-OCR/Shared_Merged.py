"""Compose UnlimitedOCR image-prefill and image-decode ONNX bundles.

UnlimitedOCR has one image-only DeepEncoder path.  The prefill shell routes a
single dynamic token-id input through both LLM_Embed and LLM_Concat_Image so the
native contiguous image placeholder span is replaced without baking a prompt.
"""
...
SHELL_PREFIXES = (
    "embed_", "concat_image_", "prefill_", "decode_", "greedy_",
    "penalty_greedy_", "sampling_",
)
    ...
    for phase in PHASES:
        for strategy in STRATEGIES:
            names[f"image_{phase}_{strategy}"] = (
                f"LLM_Image{phase.capitalize()}{''.join(part.capitalize() for part in strategy.split('_'))}.onnx"
            )
...
MERGED_CONSTITUENT_GRAPHS = (
    "LLM_Embed.onnx", "LLM_Concat_Image.onnx",
    "LLM_Rotary_Image_Prefill.onnx", "LLM_Rotary_Image_Decode.onnx",
    "LLM_Main.onnx", "LLM_Greedy.onnx", "LLM_PenaltyGreedy.onnx",
...
def load_model(path: Path) -> onnx.ModelProto:
    # Merging only needs graph headers. Streaming shared-initializer creation
    # below copies payloads directly from their original external sidecars.
...
def _external_tensor_path(source_folder: Path, tensor: TensorProto) -> tuple[Path, int, int]:
    ...
    if length_text is None:
        ...
        if tensor.data_type in _UNSHAREABLE_INIT_TYPES:
            length = (element_count + 1) // 2
        else:
            try:
                length = element_count * np.dtype(
...
def save_shared_initializers_from_external_tensors(
    ...
    """Stream raw or external tensors into one deterministic data sidecar."""
    path.parent.mkdir(parents=True, exist_ok=True)
    ...
    model = helper.make_model(graph, producer_name="UnlimitedOCR.Shared_Merged")
    model.ir_version = 10
...
def shared_external_data_map(path: Path) -> dict[str, dict[str, str]]:
    ...
    return {item.name: _external_data_map(item) for item in model.graph.initializer}
...
def write_shared_initializers_from_external_tensors(
    ...
    """Create shared storage without loading source external payloads."""
...
def write_shared_initializers(
    ...
    shared = _shareable_initializers(sources, minimum_elements, path.parent)
    if not shared:
        raise RuntimeError("No large language initializers were eligible for sharing.")
    save_shared_initializers_from_external_tensors(shared, path.parent, path)
    return shared_external_data_map(path)
...
def prefixed(model: onnx.ModelProto, prefix: str) -> onnx.ModelProto:
    ...
    return onnx.compose.add_prefix(
        model, prefix, rename_nodes=True, rename_edges=True, rename_inputs=True,
        rename_outputs=True, rename_initializers=True, rename_value_infos=True,
    )
...
def merge_models_no_check(
    ...
    """Compose two graphs without mutating either and reject real collisions."""
    ...
    merged.producer_name = "UnlimitedOCR.Shared_Merged"
    ...
    for node in merged.graph.node[second_start:]:
        for index, name in enumerate(node.input):
            if name in source_by_target:
                node.input[index] = source_by_target[name]
...
def _with_embed(source_folder: Path, names, embed: onnx.ModelProto | None = None):
    return embed if embed is not None else prefixed(
        load_model(source_folder / _file_name(names, "embed")), "embed_"
    )


def _with_rotary(source_folder: Path, phase: str, names):
    return prefixed(
        load_model(source_folder / _file_name(names, f"rotary_image_{phase}")),
        f"{phase}_",
    )


def _prefill_shell(source_folder: Path, main: onnx.ModelProto, names, embed):
    embed = _with_embed(source_folder, names, embed)
    concat = prefixed(load_model(source_folder / _file_name(names, "concat_image")), "concat_image_")
    # The target concat graph consumes the same dynamic IDs that drive LLM_Embed.
    hidden = merge_models_no_check(embed, concat, [
        ("embed_text_hidden_states", "concat_image_text_hidden_states"),
        ("embed_input_ids", "concat_image_input_ids"),
    ])
    rotary = _with_rotary(source_folder, "prefill", names)
    merged = merge_models_no_check(hidden, rotary, [])
    merged = merge_models_no_check(merged, main, [
...
def _decode_shell(source_folder: Path, main: onnx.ModelProto, names, embed):
    embed = _with_embed(source_folder, names, embed)
    rotary = _with_rotary(source_folder, "decode", names)
    ...
    rotary.graph.initializer.append(numpy_helper.from_array(
...
def _finalize(merged: onnx.ModelProto, outputs: list[str], *sources: onnx.ModelProto):
    _set_graph_outputs(merged, outputs)
    _order_state_inputs_first(merged)
    copy_metadata(merged, *sources)
    merged.producer_name = "UnlimitedOCR.Shared_Merged"
    return merged


def _build_strategy_graph(source_folder: Path, main: onnx.ModelProto, phase: str,
                          strategy: str, names, embed: onnx.ModelProto | None):
    shell, kv_seq_name, shell_sources = (
        _prefill_shell if phase == "prefill" else _decode_shell
    )(source_folder, main, names, embed)
    strategy_model = prefixed(load_model(source_folder / _file_name(names, strategy)), f"{strategy}_")
    ...
    if strategy == "greedy":
        ...
        outputs.extend(["penalty_greedy_max_logits_idx", "penalty_greedy_save_id_out", kv_seq_name])
...
def _recipe(phase: str, strategy: str):
    def build(source_folder: Path, main: onnx.ModelProto,
              model_file_names: dict[str, str] | None = None,
              embed: onnx.ModelProto | None = None) -> onnx.ModelProto:
...
def make_merged_build_plan(model_file_names: dict[str, str] | None = None):
    ...
    for phase in PHASES:
        for strategy in STRATEGIES:
            ...
            plan.append((_file_name(model_file_names, f"image_{phase}_{strategy}"),
                         _recipe(phase, strategy), dependencies))
    return plan
...
def cleanup_shadowed_constants(model: onnx.ModelProto) -> int:
    ...
    for node in model.graph.node:
        ...
        if constant is None or node.output[0] not in initializers:
            continue
        name = node.output[0]
        comparable = copy.deepcopy(constant)
        comparable.name = name
        if initializers[name].SerializeToString() != comparable.SerializeToString():
            raise RuntimeError(f"Constant output {name!r} shadows a non-identical initializer.")
        remove.add(name)
    if remove:
...
def transplant_quantized_main(target: onnx.ModelProto,
                              quantized_primary: onnx.ModelProto) -> onnx.ModelProto:
    """Replace a strategy shell's Main block with the canonical optimized Main."""
...
def validate_onnx_path(path: Path) -> None:
    ...
    fused_norms = [node for node in model.graph.node
                   if not node.domain and node.op_type == "SimplifiedLayerNormalization"]
    expected = metadata.get("fused_norm_language_count")
    if expected is not None and fused_norms and len(fused_norms) != int(expected):
        raise RuntimeError(f"{path.name} has {len(fused_norms)} fused RMSNorm nodes; expected {expected}.")
    if fused_norms:
        # Validate a transient custom-domain copy only; production nodes stay default-domain.
        for node in fused_norms:
            node.domain = "com.unlimitedocr.runtime"
        model.opset_import.append(helper.make_opsetid("com.unlimitedocr.runtime", 1))
...
def build_shared_merged_bundle(
    ...
    """Build, redirect, and validate all six image strategy graphs."""
    ...
    missing = sorted(dependency for _, _, dependencies in plan for dependency in dependencies
                     if not (source_folder / dependency).exists())
    if missing:
        raise FileNotFoundError("Cannot build a complete UnlimitedOCR image bundle; missing: "
    ...
    if shared_data_path.name != shared_path.name + ".data":
        raise RuntimeError("Shared initializer data filename must be '<model>.data'.")
    ...
    external_by_name = write_shared_initializers_from_external_tensors(
        shared_sources, source_folder, shared_path, min_shared_elements
    )
    del shared_sources
    ...
    expected = {_file_name(model_file_names, f"image_{phase}_{strategy}")
                for phase in PHASES for strategy in STRATEGIES}
...
def _external_locations(path: Path) -> set[str]:
    model = onnx.load(str(path), load_external_data=False)
    locations = {
        location for initializer in model.graph.initializer
    ...
    for node in model.graph.node:
        for attribute in node.attribute:
            if attribute.HasField('t') and attribute.t.data_location == TensorProto.EXTERNAL:
                location = _external_data_map(attribute.t).get('location')
                if location:
                    locations.add(location)
    return locations


def delete_merged_constituents(folder: Path,
                               protected_names: tuple[str, ...] | set[str] | None = None) -> list[str]:
    """Delete only absorbed constituents after every merged validator passes."""
    folder = Path(folder)
...
def attach_shared_initializers(session_options, shared_model_path: Path):
    """Attach mmap-backed shared tensors and retain all Python owners."""
    ...
    for initializer in shared.graph.initializer:
        # NumPy cannot correctly represent logical UINT4/INT4 storage.
        ...
        array = np.memmap(
            ...
            mode="r", offset=int(external.get("offset", "0")),
