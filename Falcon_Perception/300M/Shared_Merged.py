"""Build Falcon Perception image-prefill and image-decode ONNX bundles.

The language graphs are deliberately composed after export instead of being traced
as one monolith.  This keeps the image frontend reusable, makes the decoder state
block leading in every graph, and stores the large embedding/Main initializers in
one externally referenced file.
"""
...
def default_model_file_names() -> dict[str, str]:
	"""Return the complete image-only runtime filename contract."""
	names = {
	...
	for phase in PHASES:
		for strategy in STRATEGIES:
			names[f"image_{phase}_{strategy}"] = (
				f"LLM_Image{phase.capitalize()}{''.join(piece.capitalize() for piece in strategy.split('_'))}.onnx"
			)
...
def _file_name(model_file_names: dict[str, str] | None, key: str) -> str:
	...
	try:
		...
		raise KeyError(f"Missing model filename metadata for {key!r}.") from exc
...
def save_shared_initializers_from_tensors(shared: dict[str, TensorProto], path: Path) -> None:
	"""Stream shared tensor bytes into one deterministic external-data file."""
	path.parent.mkdir(parents=True, exist_ok=True)
	...
	model = helper.make_model(graph, producer_name="FalconPerception.Shared_Merged")
	model.ir_version = 10
...
def _external_tensor_path(source_folder: Path, tensor: TensorProto) -> tuple[Path, int, int]:
	...
	if length_text is None:
		raise RuntimeError(
			f"External initializer {tensor.name!r} has no byte length for streaming copy."
		)
...
def _write_initializer_bytes(
	...
	with source_path.open("rb") as source_file:
		...
		while remaining:
			chunk = source_file.read(min(chunk_bytes, remaining))
			if not chunk:
				raise RuntimeError(
					f"Unexpected EOF while copying external initializer {tensor.name!r}."
				)
...
def save_shared_initializers_from_external_tensors(
	shared: dict[str, TensorProto],
	source_folder: Path,
	path: Path,
) -> None:
	"""Write shared tensors by streaming raw or external bytes into one sidecar."""
	path.parent.mkdir(parents=True, exist_ok=True)
	data_path = path.with_name(path.name + ".data")
	temp_data_path = data_path.with_name(data_path.name + ".tmp")
	path.unlink(missing_ok=True)
	...
	offset = 0
	try:
		with temp_data_path.open("wb") as data_file:
			for name, tensor in sorted(shared.items()):
...
def write_shared_initializers_from_external_tensors(
	...
	"""Create the shared sidecar without loading external tensor payloads."""
	shared = _shareable_initializers(sources, minimum_elements, Path(source_folder))
	# An ONNX external-data save may put even tiny scales or zero points in the
	# temporary donor sidecar. They must follow the large tensors into the new
	# shared file before that sidecar is removed.
	...
	if not shared:
		raise RuntimeError("No large language initializers were eligible for sharing.")
	save_shared_initializers_from_external_tensors(shared, Path(source_folder), path)
	return shared_external_data_map(path)


def shared_external_data_map(path: Path) -> dict[str, dict[str, str]]:
	model = onnx.load(str(path), load_external_data=False)
	return {initializer.name: _external_data_map(initializer) for initializer in model.graph.initializer}
...
def merge_models_no_check(
	...
	"""Merge models without mutating either source and reject real collisions."""
	...
	merged.producer_name = "FalconPerception.Shared_Merged"
...
def _deepstack_inputs(main: onnx.ModelProto) -> list[str]:
	names = [
		item.name
		for item in main.graph.input
		if item.name.startswith("deepstack_feature_")
	]
	return sorted(names, key=lambda name: int(name.rsplit("_", 1)[1]))


def _zero_like_model(
	source_info: onnx.ValueInfoProto,
	output_name: str,
	opset: int,
) -> onnx.ModelProto:
	...
	dtype = onnx.helper.tensor_dtype_to_np_dtype(source_info.type.tensor_type.elem_type)
	zero = numpy_helper.from_array(np.asarray(0, dtype=dtype), name=f"{output_name}_scalar")
	node = helper.make_node("Mul", [input_info.name, zero.name], [output_name], name=f"{output_name}_node")
	graph = helper.make_graph([node], f"{output_name}_graph", [input_info], [output_info], [zero])
	return helper.make_model(
		graph,
		producer_name="FalconPerception.Shared_Merged",
...
def _with_zero_deepstack(
	...
	if not deepstack_inputs:
		return hidden_model, []
	source_info = value_info_by_name(hidden_model).get(hidden_name)
	if source_info is None:
		raise RuntimeError(f"Cannot build decode deepstack zeros for {hidden_name!r}.")
	default_opset = max(
		(item.version for item in hidden_model.opset_import if item.domain == ""),
		default=13,
	)
	zero_name = f"{phase}_zero_deepstack"
	zero_model = _zero_like_model(source_info, zero_name, default_opset)
	merged = merge_models_no_check(
		hidden_model,
		zero_model,
		[(hidden_name, f"{zero_name}_input")],
	)
	return merged, [(zero_name, input_name) for input_name in deepstack_inputs]
...
def _with_rotary(
	...
	key = f"rotary_image_{phase}"
	return prefixed(load_model(source_folder / _file_name(model_file_names, key)), f"{phase}_")


def _falcon_feedback_selector(source_info: onnx.ValueInfoProto) -> onnx.ModelProto:
	"""Select target-native Fourier feedback for Falcon coord/size decode tokens."""
	default_opset = 20
	embed_info = copy.deepcopy(source_info)
	embed_info.name = "falcon_feedback_embed_hidden_states"
	feedback_info = copy.deepcopy(source_info)
	feedback_info.name = "falcon_feedback_hidden_states"
	selected_info = copy.deepcopy(source_info)
	selected_info.name = "falcon_feedback_selected_hidden_states"
	use_feedback = helper.make_tensor_value_info(
		"falcon_use_feedback", TensorProto.BOOL, [1, 1, 1]
	)
	node = helper.make_node(
		"Where",
		[
			"falcon_use_feedback",
			"falcon_feedback_hidden_states",
			"falcon_feedback_embed_hidden_states",
		],
		["falcon_feedback_selected_hidden_states"],
		name="falcon_feedback_select_hidden_states",
	)
	graph = helper.make_graph(
		[node],
		"falcon_feedback_selector",
		[embed_info, feedback_info, use_feedback],
		[selected_info],
	)
	return helper.make_model(
		graph,
		producer_name="FalconPerception.Shared_Merged",
		opset_imports=[helper.make_opsetid("", default_opset)],
	)
...
def _prefill_shell(
	...
	hidden = merge_models_no_check(
		embed,
		concat,
		[("embed_text_hidden_states", "concat_image_text_hidden_states")],
	)
	deepstack_map = []
	names = set(value_info_by_name(hidden))
	for index, input_name in enumerate(_deepstack_inputs(main)):
		output_name = f"concat_image_out_deepstack_feature_{index}"
		if output_name not in names:
			raise RuntimeError(f"Concat graph did not export {output_name!r}.")
		deepstack_map.append((output_name, input_name))
...
def _decode_shell(
	...
	embed = _with_embed(source_folder, model_file_names, embed)
	hidden, deepstack_map = _with_zero_deepstack(
		embed,
		"embed_text_hidden_states",
		_deepstack_inputs(main),
		"decode",
	)
	embed_info = value_info_by_name(hidden).get("embed_text_hidden_states")
	if embed_info is None:
		raise RuntimeError("Falcon decode shell cannot find the Embed hidden-state output.")
	feedback = _falcon_feedback_selector(embed_info)
	hidden = merge_models_no_check(
		hidden,
		feedback,
		[("embed_text_hidden_states", "falcon_feedback_embed_hidden_states")],
	)
	...
	merged = merge_models_no_check(
		...
		main,
		[
			("falcon_feedback_selected_hidden_states", "hidden_states"),
	...
	return merged, "decode_kv_seq_len_next", [feedback, rotary]


def _finalize(
	merged: onnx.ModelProto,
	output_names: list[str],
	*metadata_sources: onnx.ModelProto,
) -> onnx.ModelProto:
...
def _build_strategy_graph(
	...
	available_outputs = value_info_by_name(merged)
	if "final_hidden_states" in available_outputs:
		outputs.append("final_hidden_states")
	if strategy == "greedy":
...
def write_shared_initializers(
	...
	if not shared:
		raise RuntimeError("No large language initializers were eligible for sharing.")
	save_shared_initializers_from_tensors(shared, path)
	return shared_external_data_map(path)
...
def cleanup_shadowed_constants(model: onnx.ModelProto) -> int:
	...
	for node in model.graph.node:
		...
		if initializer.SerializeToString() != comparable.SerializeToString():
			raise RuntimeError(
				f"Constant output {name!r} shadows a non-identical initializer."
			)
		remove.add(name)
	if remove:
...
def _target_main_remap(target: onnx.ModelProto) -> dict[str, str]:
	...
	if "decode_rotary_cos" in names:
		...
		if "falcon_feedback_selected_hidden_states" in names:
			remap["concat_image_concat_hidden_states"] = "falcon_feedback_selected_hidden_states"
		elif "embed_text_hidden_states" in names:
			remap["concat_image_concat_hidden_states"] = "embed_text_hidden_states"
		return remap
...
def _target_deepstack_sources(target: onnx.ModelProto) -> list[str]:
	...
	names.update(item.name for item in target.graph.initializer)
	names.update(output for node in target.graph.node for output in node.output)
	concat_sources = sorted(
		(name for name in names if name.startswith("concat_image_out_deepstack_feature_")),
...
def transplant_quantized_main(
	...
	"""Replace a target Main block with the canonical optimized Main block."""
	...
	locations = [
		...
		if (
			name == "prefill_zero_deepstack"
			or name.startswith("prefill_zero_deepstack_")
			or name.startswith("concat_image_out_deepstack_feature_")
		)
	]
	if locations:
		sources = _target_deepstack_sources(target)
		if not sources:
			raise RuntimeError("Quantized Main expects deepstack tensors absent from target shell.")
		if len(sources) == 1:
			for node, index in locations:
				node.input[index] = sources[0]
		elif len(sources) == len(locations):
			for (node, index), source in zip(locations, sources):
				node.input[index] = source
		else:
			raise RuntimeError("Deepstack tensor count differs between donor and target shells.")
...
def _validate_graph_model(model: onnx.ModelProto) -> None:
	initializer_names: dict[str, bytes] = {}
	for initializer in model.graph.initializer:
		serialized = initializer.SerializeToString()
		previous = initializer_names.setdefault(initializer.name, serialized)
		if previous != serialized:
			raise RuntimeError(f"Conflicting duplicate initializer {initializer.name!r}.")
	known = {item.name for item in model.graph.input} | set(initializer_names)
	...
	undefined = sorted(
		{
			...
			for node in model.graph.node
			for input_name in node.input
			if input_name and input_name not in known and input_name not in produced
		}
	)
	if undefined:
...
def validate_onnx_path(path: Path) -> None:
	...
	# This ORT build exposes its fused RMSNorm in the default domain, while
	# upstream ONNX has no schema for it. Check a
	# transient custom-domain view so every standard node is still validated.
	fused_norms = [
		node
		for node in model.graph.node
	...
	if fused_norms:
		for node in fused_norms:
			node.domain = "com.falcon_perception.runtime"
		model.opset_import.append(helper.make_opsetid("com.falcon_perception.runtime", 1))
...
def build_shared_merged_bundle(
	...
	"""Build all six image graphs and optionally remove validated constituents."""
	...
	if missing:
		raise FileNotFoundError(
			"Cannot build a complete Falcon Perception image bundle; missing: " + ", ".join(dict.fromkeys(missing))
		)
	...
	if shared_data_path.name != shared_path.name + ".data":
		raise RuntimeError("Shared initializer data filename must be the shared model filename plus '.data'.")
	...
	if set(graphs) != expected:
		raise RuntimeError(f"Merged image bundle mismatch: expected {sorted(expected)}, got {sorted(graphs)}.")
	...
	if delete_constituents and out_folder.resolve() == source_folder.resolve():
		result["removed_constituents"] = delete_merged_constituents(
			source_folder,
			protected_names=(shared_path.name, shared_data_path.name),
		)
	return result
...
def delete_merged_constituents(
	...
	"""Delete only absorbed graph files after all merged validators succeeded."""
	folder = Path(folder)
...
def attach_shared_initializers(session_options, shared_model_path: Path):
	"""Attach shared memmaps while retaining every owner for session lifetime."""
	import onnxruntime as ort

	model_path = Path(shared_model_path)
	shared = onnx.load(str(model_path), load_external_data=False)
	arrays: dict[str, np.ndarray] = {}
	values = []
	for initializer in shared.graph.initializer:
		# ORT owns UINT4/INT4 external initializers directly; NumPy has no exact
		# element dtype for these logical types, so do not reinterpret their bytes.
