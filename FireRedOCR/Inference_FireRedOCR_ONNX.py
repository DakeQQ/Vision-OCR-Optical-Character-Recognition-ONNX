"""Run FireRedOCR with image-only merged ONNX Runtime graphs.

The runtime reads its graph names and dimensions from LLM_Metadata.onnx. It only
opens a quantized merged bundle, then keeps KV and strategy-history OrtValues on
the execution provider between decode steps.
"""
...
DEFAULT_MODEL_FOLDER = SCRIPT_DIR / "FireRedOCR_Optimized"
...
def parse_args() -> Path:
    parser = argparse.ArgumentParser(description="Run FireRedOCR merged ONNX inference.")
    parser.add_argument(
        "--model-folder",
        type=Path,
...
def create_merged_session(
    ...
    session._firered_shared_initializers = references
    return session
...
def build_firered_prompt_tokens(
    tokenizer: Tokenizer,
    query: str,
    image_count: int,
    image_token_id: int,
    ...
    if image_count < 1:
        raise ValueError("FireRedOCR requires at least one input image.")
    ...
    prompt = (
        ...
        f"{image_placeholder * image_count}{query}{CHAT_END_TOKEN}\n"
        f"{CHAT_START_TOKEN}assistant\n"
    )
    if prompt.count(IMAGE_TOKEN) != image_count:
        raise RuntimeError("Manual FireRed chat prompt has an unexpected image placeholder count.")
    ...
    if len(positions) != expected_count or not positions:
        raise RuntimeError("FireRed chat-template image token expansion does not match metadata.")
    if positions[-1] - positions[0] + 1 != expected_count:
        raise RuntimeError("FireRed image tokens must form one contiguous prompt span.")
    ...
    if expected_start is not None and int(expected_start) != positions[0]:
        raise RuntimeError("FireRed chat-template image start differs from export metadata.")
    if expected_end is not None and int(expected_end) != positions[-1] + 1:
        raise RuntimeError("FireRed chat-template image end differs from export metadata.")
...
def load_images(
    ...
    if input_rank == 5:
        ...
        raise ValueError(f"Unsupported FireRed image input rank: {input_rank}.")
...
def run_generation(
    ...
    token_ids = build_firered_prompt_tokens(
        tokenizer,
        QUERY,
        batch_size,
...
def run_inference(model_folder: Path) -> None:
    """Run one FireRedOCR request from the selected ONNX bundle."""
